...
if (isset($_GET["REVIEW_BY_STUDENT_ID"]) && !empty($_GET['REVIEW_BY_STUDENT_ID'])) {

    $student_ID = $_GET["REVIEW_BY_STUDENT_ID"];
    $student_info = $enroll->review_student($student_ID);
}

if (isset($_POST["confirm"])) {

    $lrn = $_POST["lrn"];
    $result = $enroll->approve_student($lrn);

    if ($result == 1) {
        $_SESSION["alert"] = "Student Approved!";
        $_SESSION["alert_code"] = "success";
        // Redirect the user after a successful update
        header("Location: ../admin/enrolled_students.php");
        exit;
    }
        ...
        <div class="main-section">
            ...
            <div class="columns main-section-article">
                <div class="cols-title title-show">
                    <h4><i class="fa-solid fa-user fa-fw me-2"></i><?= ($student_info["STATUS"] == "Enrolled" || $student_info["STATUS"] == "Dropped") ? "Enrolled Student" : "Review Enrollees (<span class='badge badge-success text-success'>Input LRN for the student.</span>)";?></h4>
                </div>
                <div class="cols">
                    ...
                    <div class="cols">
                        <div class="application-details">
                            <form action="" method="post" id="enroll">
                                <div class="cols">
                                    <h5 class="pb-4">STUDENT INFORMATION 
                                        <span class="badge <?= ($student_info["STATUS"] == "Enrolled") ? "badge-success text-success" : "", 
                                        ($student_info["STATUS"] == "Dropped") ? "badge-danger text-danger" : ""; ?>"><?= $student_info["STATUS"]; ?></span>
                                    </h5>
                                </div>
                                <div class="cols col-names">
                                    <div class="form-outline mb-4">
                                        <input type="<?= ($student_info["LRN"] !== null) ? "text" : "number";?>" id="form6Example1" name="lrn" class="form-control" placeholder="LRN must be 12 numbers only" 
                                        value="<?= ($student_info["LRN"] !== null) ? $student_info["LRN"] : "";?>" 
                                        required pattern="[0-9]{12}" onKeyPress="if(this.value.length==12) return false;" 
                                        <?= ($student_info["STATUS"] == "Enrolled") ? "disabled" : "", ($student_info["STATUS"] == "Dropped") ? "disabled" : "";?>/>
                                ...
                                <!-- Submit button -->
                                <?php if ($student_info["STATUS"] == "Pending"): ?>
                                    <input type="submit" class="btn btn-primary btn-block mb-4 mt-4 btn-submit confirm" id="confirm" value="APPROVE STUDENT" name="confirm">
                                <?php endif; ?>
                            </form>
                        </div>
                    </div>
